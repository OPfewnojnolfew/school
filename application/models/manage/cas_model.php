...
class Cas_model extends CI_Model {
	...
	function queryData($title, $begin, $end, $offset, $rows, $sort, $order) {
		...
		$totalarr = $this->db->query ( 'SELECT COUNT(*) AS total FROM case' )->result ();
		$total = $totalarr [0]->total;
		$case = $this->db->query ( "SELECT * FROM case WHERE $whereCondition" )->result ();
		return "{\"total\":$total,\"rows\":" . json_encode ( $case ) . "}";
	}
	function getNews($id) {
