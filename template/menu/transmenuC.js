...
function TransMenu(oActuator, iDirection, iLeft, iTop, iReferencePoint, parentMenuSet) {
        ...
        function initEvents() {
                // hook item mouseover
                for (var i = 0, item = null; item = elmCache.item[i]; i++) {
						if (item.className == 'itemcol_static') continue;
        ...
        function highlight(oRow) {
                oRow.className = "itemcol hover";
                if (childMenus[oRow._index]) 
                        oRow.lastChild.firstChild.src = TransMenu.dingbatOn;
        }

        function dehighlight(oRow) {
                oRow.className = "itemcol";
                if (childMenus[oRow._index]) 
                ...
                if (!animating) {
					{
						...
						{
							var text = _this.items[this._index].text;
							if (text == '学生会' || text == '教务系统' || text == '国家职业技能鉴定所' || text == '同学会' || text == '图书馆' || text == '点击率查询')
							{
        ...
        function toString() {
                ...
                for (var i = 0, item = null; item = this.items[i]; i++) {
						...
						/* atoi@20070420: multi column support */
						if (i == 0)
						{
							sMenuItemString += '<tr class="item">';
						}
						else if (i % this.columns == 0)
						{
							sMenuItemString += '</tr><tr class="item">';
						}

						sMenuItemString += item.toString(childMenus[i]);
                }
				/* fix up end TR */
				for (var j = 0; j < i % this.columns; i ++)
				{
						sMenuItemString += '<td colspan="1" class="itemcol_static">&nbsp;</td>';
				}
				sMenuItemString += "</tr>";
...
function TransMenuSet(iDirection, iLeft, iTop, iReferencePoint) {
        ...
        function addMenu(oActuator, columns) {
...
function TransMenuItem(sText, sUrl, oParent) {
        ...
        function toString(bDingbat) {
				...
				/* atoi@20070420: remove tr for multi column support */
				if (this.text == '　　')
					return '<td nowrap style="' + sPaddingLeft + '" class="itemcol_static">' + sText + '</td>';
				else
					return '<td nowrap style="' + sPaddingLeft + '" class="itemcol">' + 
                        sText + '</td>';
        }
}
